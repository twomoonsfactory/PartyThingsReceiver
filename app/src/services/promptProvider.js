export default class promptProvider{
  constructor($log, eventService, gameEvents, gameStates, $http){
    ...
    this.prompt = "";
    this.votes = [];
    //local reference to injected externals
    this.log = $log;
    ...
    this.http = $http;
    //event subscriptions
    this.eventService.subscribe(this.gameEvents.welcomeLoaded, this.loadPrompts);
    this.eventService.subscribe(this.gameStates.WaitingForReady, this.getPrompts);
    this.eventService.subscribe(this.gameStates.RoundEnd, this.getPrompts);
  }
  loadPrompts(){
