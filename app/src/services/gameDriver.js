export default class gameDriver{
  constructor(eventService, gameEvents, stateManager, gameStates, messageSender, messageProvider, messageNames, playerHandler, playerStates, responseHandler, promptProvider, guessHandler, $log){
    ...
    //event subscriptions
    this.eventService.subscribe(this.gameStates.WaitingForReady, this.readyUp);
    this.eventService.subscribe(this.gameEvents.readyReceived, this.playerReady);
    this.eventService.subscribe(this.gameStates.ReadyToStart, this.sendPrompts);
    this.eventService.subscribe(this.gameEvents.voteReceived, this.voteReceived);
    this.eventService.subscribe(this.gameStates.PromptChosen, this.requestResponse);
    this.eventService.subscribe(this.gameEvents.responseReceived, this.receivedResponse);
    this.eventService.subscribe(this.gameStates.ResponsesReceived, this.startGuessing);
    this.eventService.subscribe(this.gameEvents.guessReceived, this.guessReceiver);
    this.eventService.subscribe(this.gameEvents.guessesResolved, this.guessesResolved);
    this.eventService.subscribe(this.gameStates.RoundEnd, this.nextRound);
    this.eventService.subscribe(this.gameStates.GameEnd, this.endGame);
  }
  ...
  playerReady(args){
    ...
    this.eventService.publish(gameEvents.playerUpdated, "");
  ...
  sendPrompts(){
    _.each(this.playerHandler.players, player => {
      if(player.checkState(this.playerStates.ready)){
        messageSender.requestPrompt({senderId: player.senderId, message: {message: this.messageProvider.getMessage({messageName: this.messageNames.promptRequest, pname: player.playerName}), prompts: this.promptProvider.currentprompts}});
  ...
  startGuessing(){
    _.each(this.playerHandler.players, player => {
      if(player.checkState(this.playerStates.ready)){
        this.messageSender.requestGuess({senderId: player.senderId,message:{message: this.messageProvider.getMessage({messageName:this.messageNames.guessRequest}), things: this.responseHandler.getResponses(), elegiblePlayers: this.playerHandler.getElegiblePlayers()}});
  ...
  guessReceiver(args){
    ...
    this.guesser.setState(this.playerStates.ready);
...
gameDriver.$inject = ['eventService', 'gameEvents', 'stateManager', 'gameStates', 'messageSender', 'messageProvider', 'messageNames', 'playerHandler', 'playerStates', 'responseHandler', 'promptProvider', 'guessHandler', '$log'];
