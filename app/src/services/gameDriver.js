export default ngModule =>{
  ngModule.service('gameDriver', ['eventService', 'gameEvents', 'stateManager', 'gameStates', 'messageSender', 'messageProvider', 'messageNames', 'playerHandler', 'playerStates', 'responseHandler', 'promptProvider', 'guessHandler', '$log', (eventService, gameEvents, stateManager, gameStates, messageSender, messageProvider, messageNames, playerHandler, playerStates, responseHandler, promptProvider, guessHandler, $log) => {
        let self = this;
        ...
        this.readyUp = ()=>{
          _.each(playerHandler.players, player => {
        ...
        this.nextRound = ()=>{
          if(playerHandler.highScore()>=self.winningScore){
            ...
            let score = playerHandler.highScore();
          ...
          _.each(playerHandler.players, player => {
            ...
            if(player.checkState(playerStates.ready)){
              if(_.contains(stateManager.winners, player)){
                endMessage += messageProvider.getMessage({messageName: messageNames.winner, pname: player.playerName, points: playerHandler.highScore()});
              }
